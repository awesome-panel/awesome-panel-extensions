...
def options(where):
    ...
    return gv.tile_sources.OSM * data.hvplot.quadmesh(
        rasterize=True, geo=True, colormap="viridis", project=True
    ).opts(alpha=0.7)


pn.Row(pn.interact(options, where=places), greatest, least).servable()
